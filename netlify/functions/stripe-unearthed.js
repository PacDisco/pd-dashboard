// netlify/functions/stripe-unearthed.js
//
// Proxies the Stripe REST API so the secret key never ships to the browser.
//
// Endpoints:
//   GET /api/stripe-unearthed?action=charges&limit=100&starting_after=ch_xxx&created_gte=...&created_lte=...
//       -> list charges with optional pagination + date filter (unix seconds)
//   GET /api/stripe-unearthed?action=charge&id=ch_xxx
//       -> full charge object (expanded with customer + balance_transaction + payment_intent)
//   GET /api/stripe-unearthed?action=summary&created_gte=...&created_lte=...
//       -> aggregate KPIs for the given window (paginates charges internally)
//
// Environment:
//   STRIPE_SECRET_KEY_UNEARTHED  — required. Set in Netlify Site settings → Environment variables.
//
...
export const handler = async (event) => {
  ...
  const key = process.env.STRIPE_SECRET_KEY_UNEARTHED;
  if (!key) {
    return send(500, {
      error: "STRIPE_SECRET_KEY_UNEARTHED is not set on this site. Add it under Site configuration → Environment variables and redeploy.",
    });
  }
